# Agent Script Implementation Guide

## Overview

The updated Agentforce Implementation Guide is the hands-on path for building agents on the Salesforce Platform.

## What changed

- The guide is updated for the new Agentforce Builder in Agentforce Studio.
- It walks through Agent Script for a more deterministic agent experience.
- It highlights hybrid reasoning, preview and debugging improvements, and the new Agentforce Testing Center.
- It also includes an updated Observability guide.

## Setup Notes

- Focuses on Agentforce Builder in Agentforce Studio.
- Available starting April 20, 2026.

## Notes for Agents

- This is the best starting point when the goal is to build or validate a customer-facing agent.
